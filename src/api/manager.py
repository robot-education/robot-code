...
class CodeManager:
    def __init__(
        self, document_path: onshape.Path, client: client.FeatureStudioClient
    ) -> None:
        self._document_path = document_path
        self._client = client

        self._FOLDER_PATH = "./fs_code"
        self._ID_FILE = "./.storage/id_file.json"
        self._OUTDATED_VERSION_MATCH: re.Pattern[Any] = re.compile(
    ...
    def push(self) -> None:
        ...
        for id, name in items:
            ...
            path = onshape.Path(self._document_path.did, self._document_path.wid, id)
    ...
    def update_std(self) -> None:
        ...
        files = os.listdir(path=self._FOLDER_PATH)
    ...
    def _update_version(self, contents: str, std_version: str) -> str:
        return re.sub(
            pattern=self._OUTDATED_VERSION_MATCH,
...
def make_manager(logging: bool = False) -> CodeManager:
    feature_client = client.FeatureStudioClient(onshape.Onshape(logging=logging))
    return CodeManager(constants.BACKEND_PATH, feature_client)
