...
from onshape_api.utils import str_utils, endpoint_utils
...
def release_preamble(version_name: str, studio_path: paths.ElementPath) -> str:
    return RELEASE_PREAMBLE.format(version_name, paths.path_to_url(studio_path))
...
def release(
    ...
    studio = endpoints.get_feature_studio(api, backend, studio_name)
    if not studio:
    ...
    endpoint_utils.confirm_version_creation(new_version_name)
    new_version = versions.create_version(api, backend, new_version_name, description)

    update_release_studio(api, config, studio, new_version_name, new_version["id"])
    versions.create_version(api, frontend, new_version_name, description)
...
def update_release_studio(
    ...
    """
    version_path = paths.ElementPath(
    ...
    endpoints.push_studio(api, config.documents["frontend"], studio.name, release_code)
