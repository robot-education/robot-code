...
const actionInfo: ActionInfo = {
    title: "Push version recursively",
    description:
        "Create a version and push it to one or more linked parent documents. recursively",
    route: "push-version-recursive"
};

export function PushVersionRecursiveCard() {
    return <ActionCard actionInfo={actionInfo} />;
}

interface PushVersionRecursiveArgs {
    name: string;
    description: string;
    instancePaths: WorkspacePath[];
}

export function PushVersionRecursive() {
    const pushVersionRecursiveMutationFn = async (args: PushVersionRecursiveArgs) => {
        return post("/push-version-recursive" + currentInstanceApiPath(), {
            body: {
                name: args.name,
    ...
    const mutation = useMutation({
        ...
        mutationFn: pushVersionRecursiveMutationFn
    });
    ...
    if (mutation.isSuccess) {
        ...
        actionSuccess = (
            <ActionSuccess
                message="Successfully pushed versions"
    ...
    return (
        <ActionDialog title={actionInfo.title} isPending={mutation.isPending}>
            {mutation.isIdle && (
                <PushVersionRecursiveForm onSubmit={mutation.mutate} />
            )}
            {mutation.isPending && (
...
function PushVersionRecursiveForm(
    props: OnSubmitProps<PushVersionRecursiveArgs>
) {
