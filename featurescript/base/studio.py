...
class Studio(node.ParentNode):
    ...
    def __init__(self, studio_name: str, import_common: bool = True) -> None:
        self.studio_name = studio_name
        self.std_imports: list[imp.StdImport] = []
        self.imports: list[imp.Import] = []
        self.children: list[node.Node] = []
        if import_common:
            self.std_imports.append(imp.StdImport("common.fs"))

    def add_import(self, imp: imp.Import) -> Self:
        self.imports.append(imp)
        return self

    def add_std_import(self, std_import: imp.StdImport) -> Self:
        self.std_imports.append(std_import)
        return self
    ...
    def _build_header(self, context: ctxt.Context) -> str:
        header = _FS_HEADER.format(str(context.std_version))
